...
import { FaCircle, FaTerminal, FaDatabase, FaLock, FaBolt } from 'react-icons/fa';
...
const TechShowcase = () => {
  ...
  return (
    <section className="py-24 px-6 relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-repeat opacity-[0.02] pointer-events-none" 
           style={{ backgroundImage: 'linear-gradient(90deg, #00ff88 1px, transparent 1px), linear-gradient(180deg, #00ff88 1px, transparent 1px)', backgroundSize: '40px 40px' }}>
      </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col items-center mb-24 text-center">
            <div className="flex items-center gap-3 mb-4 opacity-50">
               ...
               <span className="text-accent-tertiary font-mono text-[9px] tracking-[0.4em] uppercase font-bold">System_Logistics</span>
               <div className="w-8 h-px bg-accent-tertiary"></div>
            </div>
        ...
        <div className="flex flex-col lg:flex-row items-center gap-20">
          ...
          <div className="flex-1 w-full space-y-10">
            {architectures.map((item, i) => (
              <div key={i} className="group relative flex gap-6 p-6 border border-white/5 bg-background/50 hover:bg-card/40 hover:border-accent-tertiary/30 cyber-chamfer-sm transition-all duration-700">
                ...
                <div className="flex-1">
                  <h4 className="text-lg md:text-xl font-heading font-black text-white uppercase tracking-widest mb-2 group-hover:text-accent-tertiary transition-colors">{item.label}</h4>
                  <p className="text-[10px] font-mono text-foreground/40 uppercase leading-relaxed tracking-wide group-hover:text-foreground/80 transition-colors">{item.desc}</p>
                </div>
                ...
                <div className="absolute right-6 top-6 flex items-center gap-2 opacity-30 group-hover:opacity-100 transition-opacity">
                   ...
                   <span className="text-[7px] font-mono text-accent-tertiary uppercase tracking-widest">OK</span>
                </div>
              </div>
            ))}
          </div>
              ...
              <div className="bg-black/80 border border-accent-tertiary/30 cyber-chamfer-sm overflow-hidden relative group backdrop-blur-xl">
                ...
                <div className="bg-white/5 px-6 py-3 flex items-center justify-between border-b border-accent-tertiary/20 backdrop-blur-md">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-[9px] font-mono text-accent-tertiary font-bold uppercase tracking-[0.3em]">MERN_SERVER.core</span>
                  </div>
                  ...
                  <pre className="text-foreground/60 relative">
                    <code className="block w-full">
                      <div className="hover:bg-white/5 px-2 -mx-2 rounded transition-colors duration-300">
                         <span className="text-accent-secondary">import</span> <span className="text-white">{'{'} IntelligenceCore {'}'}</span> <span className="text-accent-secondary">from</span> <span className="text-accent">'@ai/nexus'</span>;
                      </div>
                      <div className="hover:bg-white/5 px-2 -mx-2 rounded transition-colors duration-300 mt-2">
                         <span className="text-accent-tertiary">const</span> <span className="text-white">ServerEnv</span> = <span className="text-accent-tertiary">await</span> <span className="text-white">Core</span>.<span className="text-accent">initSystem</span>(<span className="text-accent">{'{'}</span>
                      </div>
                      <div className="hover:bg-white/5 px-2 -mx-2 rounded transition-colors duration-300">
                         &nbsp;&nbsp;architecture: <span className="text-accent">'HIGH_AVAILABILITY'</span>,
                      </div>
                      <div className="hover:bg-white/5 px-2 -mx-2 rounded transition-colors duration-300">
                         &nbsp;&nbsp;auth_mode: <span className="text-white">env</span>.<span className="text-accent-secondary">ZERO_TRUST</span>,
                      </div>
                      <div className="hover:bg-white/5 px-2 -mx-2 rounded transition-colors duration-300">
                         &nbsp;&nbsp;auto_scale: <span className="text-accent-tertiary">true</span>
                      </div>
                      <div className="hover:bg-white/5 px-2 -mx-2 rounded transition-colors duration-300">
                         <span className="text-accent">{'}'}</span>);
                      </div>
                      <div className="mt-4 hover:bg-white/5 px-2 -mx-2 rounded transition-colors duration-300">
                         <span className="text-foreground/30">// EXECUTING NEURAL COMPILATION...</span>
                      </div>
                      <div className="hover:bg-white/5 px-2 -mx-2 rounded transition-colors duration-300">
                         <span className="text-white">ServerEnv</span>.<span className="text-accent">deploy</span>();
                      </div>
                      <div className="flex items-center mt-2 px-2 -mx-2">
                         <span className="text-accent-tertiary mr-2">&gt;</span>
                         <span className="text-accent font-bold group-hover:animate-pulse">SYSTEM_ONLINE_&_VERIFIED</span>
